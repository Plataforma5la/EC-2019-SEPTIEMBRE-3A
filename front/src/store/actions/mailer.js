...
export const sendMail = function(direccion, cart) {
  return function(dispatch, getState) {
    ...
    axios
      .post("/api/mailer", { HTML_MESSAGE: HTML_MESSAGE })
      .then(() =>
        alert(
          "Gracias por tu compra! Te hemos enviado los detalles a tu casilla de correo"
        )
      );
  };
};
...
export const sendMailAccept = function(email) {
  return function(dispatch, getState) {
    ...
    HTML_MESSAGE += `<b>Su producto ya esta despachado y sera entregado en 69 días.</b></div>`;
    return axios.post("/api/mailer/confirm", {
...
export const sendMailCancel = function(email) {
  return function(dispatch, getState) {
    ...
    return axios.post("/api/mailer/cancel", {
      HTML_MESSAGE: HTML_MESSAGE,
      email
    });
  };
